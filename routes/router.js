...
// router.post('/weather', async (req, res) => {
  ...
  // });
    ...
    //   const existingLocation = await locationCollection.findOne({ name: result.city });
    //   if (existingLocation) {
    //     await locationCollection.updateOne({ name: result.city }, { $set: { conditions: result.conditions,description:result.description,icon:result.icon, date: result.datetime,tempmax:result.tempmax,tempmin:result.tempmin, temp: result.temp,humidity:result.humidity, imageUrl: result.imageUrl,reviews:[],rating:0 } });
    //   } else {
    //     await locationCollection.insertOne({ name: result.city, conditions: result.conditions,description:result.description,icon:result.icon, date: result.datetime,tempmax:result.tempmax,tempmin:result.tempmin, temp: result.temp,humidity:result.humidity, imageUrl: result.imageUrl,reviews:[],rating:0  });
    //   }
